...
VS_FIELDPARAMETERSOUTPUT GSFieldsStart(inout VS_FIELDPARAMETERSOUTPUT InOut)
{
	...
	else if(InOut.index % 2)
	{
		InOut.size		= 8;
		float _value	= (PI / 24.5) * InOut.index;
		InOut.pos		= float3(70 * cos(_value), 70 * sin(_value),0);
		InOut.type		= 3;
	...
	{
		InOut.size		= 8;
		float _value	= (PI / 24.5) * (InOut.index - 1);
		InOut.pos		= float3(70 * cos(_value), 70 * sin(_value),0);
		InOut.type		= 1;
		InOut.goal		= -InOut.pos;
		InOut.strength	= float3(5,10,1);
	}
...
{
	float _length = length(InOut.goal);
	//if(InOut.index == 0) InOut.goal = float3(_length * cos(g_time * (1 + InOut.index)), _length * sin(g_time * (1 + InOut.index)), 0) * saturate(InOut.size);
	return InOut;
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
