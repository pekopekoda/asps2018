...
{
	if(InOut.index == 0) // directional
	{
		InOut.size		= 12;
		InOut.pos		= float3(-30,70,0);
		InOut.type		= 1;
		InOut.goal		= float3(-1,80,0);
		InOut.strength	= float3(2,6,1);
	}
	else if(InOut.index == 1) // emitter
	{
		InOut.size		= 8;
		InOut.pos		= float3(-30,70,0);
		InOut.type		= 3;
		InOut.goal		= float3(0,0,0);
		InOut.strength	= float3(1,0.1,1);
	}
	else if(InOut.index == 2) // directional
	{
		InOut.size		= 12;
		InOut.pos		= float3(30,70,0);
		InOut.type		= 1;
	...
	else if(InOut.index == 4)// bouncer
	{
		InOut.size		= 20;
		InOut.pos		= float3(1,80,0);
		InOut.type		= 0;
		InOut.goal		= float3(1,0,0);
		InOut.strength	= float3(0.75,0.1,1);
	}
	else if(InOut.index == 5)// bouncer
	{
		InOut.size		= 20;
		InOut.pos		= float3(-1, 80,0);
		InOut.type		= 0;
	...
	else if(InOut.index == 6)// portal
	{
		InOut.size		= 15;
		InOut.pos		= float3(0,70,0);
		InOut.type		= 2;
		InOut.goal		= float3(0,-150,0);
		InOut.strength	= float3(1,0.1,1);
	}
	else if(InOut.index == 7)// bouncer
	{
		InOut.size		= 15;
		InOut.pos		= float3(-50, 130,0);
		InOut.type		= 0;
		InOut.goal		= float3(1,0,0);
		InOut.strength	= float3(0.75,0.1,1);
	}
	else if(InOut.index == 8)// bouncer
	{
		InOut.size		= 15;
		InOut.pos		= float3(50, 130,0);
		InOut.type		= 0;
	...
	else if(InOut.index % 3)
	{
		InOut.size		= -1;
		InOut.pos		= float3(50 + InOut.index * 4, 80, 0);
		InOut.type		= 1;
		InOut.goal		= float3(0,-10,0);
		InOut.strength	= float3(1,1,1);
	}
	else
	{
		InOut.size		= -1;
		InOut.pos		= float3(50 + InOut.index * 4, 100, 0);
		InOut.type		= 0;
		InOut.goal		= float3(1,0,0);
		InOut.strength	= float3(1,0.1,1);
	}
...
VS_FIELDPARAMETERSOUTPUT UpdateFieldsWithScenario(inout VS_FIELDPARAMETERSOUTPUT InOut)
{
	...
	if(InOut.index == 0 || InOut.index == 2)	 //Goals
	{
		_length = length( InOut.goal );
		_length			= lerp( 0.01, _length, sign(_length) );
		InOut.dummy.xy	= (InOut.goal.xy / _length);							  //Dont't use hlsl normalize() to prevent division by zero
		...
		_rotater.x		= InOut.goal.x * _cos - InOut.goal.y * _sin;
		_rotater.y		= InOut.goal.x * _sin + InOut.goal.y * _cos;
		InOut.goal.xy	= _rotater;
		//Object speed is increased or decreased thanks to gravity and acceleration mix
		InOut.dummy.w	= _speed;
	}

	else if(InOut.index == 4 || InOut.index == 5)//Fields
	{
