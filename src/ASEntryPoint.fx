...
VS_FIELDPARAMETERSOUTPUT GSFieldsStart(inout VS_FIELDPARAMETERSOUTPUT InOut)
{
	...
	if(InOut.index == 0)
	{
		...
		InOut.goal		= float3(0,1,0);
		InOut.strength	= float3(1,1,1);
	}
	else if(InOut.index %2) //directional fields
	{
		InOut.size		= 8;
		InOut.pos		= float3(50 + InOut.index * 8, 130, 0);
	...
	{
		InOut.size		= 8;
		InOut.pos		= float3(50 + InOut.index * 8, 100, 0);
		InOut.type		= BOUNCER_TYPE;
